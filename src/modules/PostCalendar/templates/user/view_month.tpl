        ...
        <td class="{$stylesheet}"{if $in_new_window eq false} onclick="window.location.href='{pc_url action=day date=$date}';"{/if}>
            ...
            <div class="monthview_events">
                ...
                {foreach name=events item=event from=$S_EVENTS.$date}
                    ...
                    {assign var="desc" value=$event.hometext|notifyfilters:'postcalendar.hook.eventsfilter.ui.filter':$zikula_view|safetext|truncate:255:"..."}
                    {if $event.privateicon}{img src='locked.png' modname='core' set='icons/extrasmall' __title="private event" __alt="private event"}{/if}
                    {pc_url full=true action=detail eid=$event.eid date=$date style="font-size: 7pt; text-decoration: none;" title=$event.hometext|notifyfilters:'postcalendar.hook.eventsfilter.ui.filter':$zikula_view|safetext display="$timestamp $title"|safehtml}
