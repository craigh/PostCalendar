        ...
        {foreach name='eventloop' key='id' item='event' from=$todaysEvents}
            ...
            {if $eventcount < $blockVars.pcbeventslimit}
                ...
                <li class="pc_blockevent">
                    ...
                    {pc_url full=true class="eventlink" action="event" eid=$event.eid date=$event.eventStart->format($modvars.PostCalendar.pcDateFormats.date) title=$event.hometext|notifyfilters:'postcalendar.hook.eventsfilter.ui.filter'|safehtml display="$timestamp `$event.title`"|strip_tags}
